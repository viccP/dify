const translation = {
  ...
  account: {
    ...
    langGeniusAccount: 'Bonc LOps  账号',
  ...
  members: {
    ...
    invitationSentTip: '邀请已发送，对方登录 Bonc LOps  后即可访问你的团队数据。',
    invitationLink: '邀请链接',
  ...
  provider: {
    ...
    openaiHosted: {
      ...
      onTrial: '体验',
      exhausted: '超出限额',
      desc: '托管 OpenAI 由 Bonc LOps  提供的托管 OpenAI 服务，你可以使用 GPT-3.5 等模型，在体验额度消耗完毕前你需要设置其它模型库。',
      callTimes: '调用次数',
      usedUp: '试用额度已用完，请在下方添加自己的模型库',
      useYourModel: '当前正在使用你自己的模型库。',
      close: '关闭',
    },
    anthropicHosted: {
  ...
  apiBasedExtension: {
    title: 'API 扩展提供了一个集中式的 API 管理，在此统一添加 API 配置后，方便在 Bonc LOps  上的各类应用中直接使用。',
    link: '了解如何开发您自己的 API 扩展。',
    linkUrl: 'https://docs.Bonc LOps .ai/v/zh-hans/advanced/api_based_extension',
    add: '新增 API 扩展',
    selector: {
      title: 'API 扩展',
  ...
  about: {
    changeLog: '更新日志',
    updateNow: '现在更新',
    nowAvailable: 'Bonc LOps  {{version}} 现已可用。',
