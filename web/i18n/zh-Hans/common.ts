const translation = {
  ...
  account: {
    ...
    langGeniusAccount: 'Airita 账号',
    langGeniusAccountTip: '您的 Airita 账号和相关的用户数据。',
    editName: '编辑名字',
  ...
  members: {
    ...
    invitationSentTip: '邀请已发送，对方登录 Airita 后即可访问你的团队数据。',
    invitationLink: '邀请链接',
  ...
  provider: {
    ...
    openaiHosted: {
      ...
      onTrial: '体验',
      exhausted: '超出限额',
      desc: '托管 OpenAI 由 Airita 提供的托管 OpenAI 服务，你可以使用 GPT-3.5 等模型，在体验额度消耗完毕前你需要设置其它模型供应商。',
      callTimes: '调用次数',
  ...
  apiBasedExtension: {
    title: 'API 扩展提供了一个集中式的 API 管理，在此统一添加 API 配置后，方便在 Airita 上的各类应用中直接使用。',
    link: '了解如何开发您自己的 API 扩展。',
    linkUrl: 'https://docs.Airita.ai/v/zh-hans/guides/extension/api-based-extension',
    add: '新增 API 扩展',
    selector: {
      title: 'API 扩展',
  ...
  about: {
    changeLog: '更新日志',
    updateNow: '现在更新',
    nowAvailable: 'Airita {{version}} 现已可用。',
    latestAvailable: 'Airita {{version}} 已是最新版本。',
  },
  appMenus: {
    overview: '监测',
    promptEng: '编排',
