...
import {
  ...
  fetchCollectionList as doFetchCollectionList,
...
const Tools: FC<Props> = ({
  ...
  const [currCollectionIndex, setCurrCollectionIndex] = useState<number | null>(
    null
  );
  ...
  const [toolPageCollectionType, setToolPageCollectionType] =
    useTabSearchParams({
      defaultTab: collectionTypeOptions[0].value,
    });
  const [appPageCollectionType, setAppPageCollectionType] = useState(
    collectionTypeOptions[0].value
  );
  ...
  const showCollectionList = (() => {
    ...
    if (collectionType === CollectionType.all)
      typeFilteredList = collectionList.filter(
        (item) => item.type !== CollectionType.model
      );
    else if (collectionType === CollectionType.builtIn)
      typeFilteredList = collectionList.filter(
        (item) => item.type === CollectionType.builtIn
      );
  ...
  const hasNoCustomCollection = !collectionList.find(
    (item) => item.type === CollectionType.custom
  );
  ...
  const currCollection = (() => {
    if (currCollectionIndex === null) return null;
  ...
  const [isShowEditCollectionToolModal, setIsShowEditCollectionToolModal] =
    useState(false);
  ...
  return (
    <>
      <div className="flex h-full">
        ...
        <div
          className={cn(
            isInToolsPage ? "sm:w-[216px] px-4" : "sm:w-[256px] px-3",
            "flex flex-col  w-16 shrink-0 pb-2"
          )}
        >
          {isInToolsPage && (
            <Button
              className="mt-6 flex items-center !h-8 pl-4"
              type="primary"
              onClick={handleCreateToolCollection}
            >
              <Plus className="w-4 h-4 mr-1" />
              <div className="leading-[18px] text-[13px] font-medium truncate">
          ...
          {isInDebugPage && (
            <div className="mt-6 flex space-x-1 items-center">
              <Search className="grow" value={query} onChange={setQuery} />
              <Button
          ...
          />
          {isInToolsPage && (
            <Search className="mt-5" value={query} onChange={setQuery} />
          )}
        ...
        <div className={cn("grow h-full overflow-hidden p-2")}>
          <div className="h-full bg-white rounded-2xl">
            ...
            {collectionType === CollectionType.custom &&
              hasNoCustomCollection && <NoCustomToolPlaceholder />}
          </div>
        </div>
      </div>
